...
for stock_code in stock_codes:
    ...
    for index, stock_daily_info in stock_daily_history.iterrows():
        ...
        # 确定买入数量，第一次建仓买入十分之一
        number = util.can_buy_num(user_account.get_total_asset() / 10, close_price)

        # 首次买入
        if table.length == 0:
            print("日期：", date, "pe_ttm: ", pe_ttm)
        ...
        for grid_record_key, grid_record in enumerate(grid_records):
            ...
            if close_price >= sell_price:
                print("日期：", date, "pe_ttm: ", pe_ttm)
                # 满足卖点，卖出
                user_account.sell(stock_code, close_price, grid_record['stock'].holding_num)
